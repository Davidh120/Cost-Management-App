<template>
  <div class="container-modal">
    ...
    <form @submit.prevent="onSubmit">
      <div class="user-box">
        <input type="text" name="name" required v-model="nameUp" />
        <label>Name</label>
      </div>
      <div class="user-box">
        <input type="number" name="cost" required v-model="costUp" />
        <label>Cost</label>
      </div>
      <div class="user-box">
        <select class="type" v-model="typeUp">
...
export default {
  ...
  data: function () {
    return {
      id: this.costUpdate.id,
      nameUp: this.costUpdate.costData.name,
      costUp: this.costUpdate.costData.cost,
      typeUp: this.costUpdate.costData.type,
  ...
  methods: {
    async update() {
      try {
        let nameField = this.nameUp.replace(/^\s+/g, "");
        if (nameField === "") {
          ...
          this.errorMsg= ""
          await updateDoc(doc(db,this.mainCollection,this.user,this.costsCollection,this.id),
            {
              name: nameField,
              cost: this.costUp,
              type: this.typeUp,
